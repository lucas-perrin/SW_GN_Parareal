...
compute = 1;
if compute
clearvars -except compute parallel
end
close all
...
flim   = 0;
frames = 20; % nb of frames for the movie
...
fprintf(['size Wf SW = ',num2str(size(SW_sol{end})),'\n'])
...
fprintf('computing parallel... \n')
[U_para, t_para] = Parareal_v3(W0,[0, Tj_list],@Green_Nagdhi,@Shallow_Water);
fprintf('time Para : %d sec \n\n',t_para(end))
...
error_L2_h = zeros(N+1,N+1);
error_entropy = zeros(N+1,N+1);
SWerror_L2_h = zeros(N+1,N+1);
baseline_err_L2_h = zeros(N+1,N+1);
